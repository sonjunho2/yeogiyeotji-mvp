...
- JSON 로컬 모드에서는 세션을 프로세스 메모리에만 저장합니다. 서버를 재시작하면 세션이 사라지므로 다시 로그인해야 하며, `server/data/store.json`에는 세션이 기록되지 않습니다.
- PostgreSQL 모드에서는 세션을 PostgreSQL `sessions` 테이블에 저장하므로 서버 재시작이나 재배포 후에도 만료 전까지 로그인을 유지할 수 있습니다. 브라우저 쿠키의 원본 세션 토큰은 데이터베이스에 저장하지 않고, SHA-256 token hash만 저장합니다. 세션 유효기간은 7일입니다.
...
이 인증은 운영 인증이 완성된 상태가 아닌 기반 구현입니다. 남아 있는 운영 보안 과제는 이메일 인증, 비밀번호 재설정, 로그인 시도 제한, 보안 모니터링입니다.
