...
async function assertLinkFunctionStaticScope() { const source = fs.readFileSync('apps/web-prototype/auth-controller.js', 'utf8'); const linkStart = source.indexOf('async function linkExistingAccount'); const linkEnd = source.indexOf('\n    return { getState:', linkStart); assert.notEqual(linkStart, -1); assert.notEqual(linkEnd, -1); assert.ok(linkEnd > linkStart); const linkFunctionSource = source.slice(linkStart, linkEnd); assert.equal((linkFunctionSource.match(/인증이 만료되었습니다\. 인증을 종료한 뒤 다시 시도해 주세요\./g) || []).length, 3); assert.equal((linkFunctionSource.match(/Google 인증이 만료되었습니다\./g) || []).length, 0); }

async function assertBearerErrorCleanupMappings() {
  for (const code of ['bearer_required', 'invalid_authorization', 'invalid_token']) {
    const sequence = []; const counters = { login: 0, linkSupabaseAccount: 0, getCurrentUser: 0, loadServerData: 0, logoutLegacySession: 0, onAuthenticated: 0 };
    const error = { code, message: 'RAW_ERROR_SENTINEL' };
    const t = setup({ sequence, dataStore: { login: async () => { counters.login += 1; sequence.push('login'); }, linkSupabaseAccount: async () => { counters.linkSupabaseAccount += 1; sequence.push('linkSupabaseAccount'); throw error; }, getCurrentUser: async () => { counters.getCurrentUser += 1; sequence.push('getCurrentUser'); return { id: 'unexpected-user' }; }, loadServerData: async () => { counters.loadServerData += 1; sequence.push('loadServerData'); return { places: ['unexpected-place'], collections: ['unexpected-collection'] }; }, logoutLegacySession: async () => { counters.logoutLegacySession += 1; sequence.push('logoutLegacySession'); } }, onAuthenticated: () => { counters.onAuthenticated += 1; sequence.push('onAuthenticated'); } });
    t.controller.handleInitialIssue('auth_user_not_linked'); assert.equal(await t.controller.linkExistingAccount('existing@example.com', 'PASSWORD_SECRET_SENTINEL_5_3'), null);
    assert.deepEqual(sequence, ['login', 'linkSupabaseAccount', 'logoutLegacySession']); assert.deepEqual(counters, { login: 1, linkSupabaseAccount: 1, getCurrentUser: 0, loadServerData: 0, logoutLegacySession: 1, onAuthenticated: 0 }); assert.equal(t.controller.getState().error, '인증이 만료되었습니다. 인증을 종료한 뒤 다시 시도해 주세요.'); assert.equal(t.controller.getState().pending, false); assert.equal(t.controller.getState().mode, 'otp-unlinked'); assert.equal(t.controller.getState().issue, ''); assertNoLinkSensitiveData(t.controller.getState(), t.changes, [error.message]);
  }
}
...
async function run() {
  ...
  await assertLinkFunctionStaticScope();
  await assertBearerErrorCleanupMappings();
