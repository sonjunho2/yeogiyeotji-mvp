...
function createPostgresStorage({ pool }) {
  ...
  return {
    async initialize() { const result = await query("SELECT to_regclass('users') AS users, to_regclass('collections') AS collections, to_regclass('places') AS places"); if (!result.rows[0].users || !result.rows[0].collections || !result.rows[0].places) throw new Error('PostgreSQL schema is not migrated'); },
