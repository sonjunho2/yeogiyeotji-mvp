...
(async () => {
  ...
  assert.equal((await testPool.query("SELECT version FROM schema_migrations WHERE version = '003'")).rowCount, 1);
  assert.equal((await testPool.query("SELECT data_type FROM information_schema.columns WHERE table_schema = current_schema() AND table_name = 'users' AND column_name = 'auth_user_id'")).rows[0].data_type, 'uuid');
  assert.equal((await testPool.query("SELECT is_nullable FROM information_schema.columns WHERE table_schema = current_schema() AND table_name = 'users' AND column_name = 'auth_user_id'")).rows[0].is_nullable, 'YES');
  assert.equal((await testPool.query("SELECT indexname FROM pg_indexes WHERE schemaname = current_schema() AND indexname = 'users_auth_user_id_uidx'")).rowCount, 1);
  assert.equal((await testPool.query("SELECT COUNT(*)::int AS count FROM information_schema.table_constraints tc JOIN information_schema.key_column_usage kcu ON tc.constraint_catalog = kcu.constraint_catalog AND tc.constraint_schema = kcu.constraint_schema AND tc.constraint_name = kcu.constraint_name WHERE tc.constraint_schema = current_schema() AND tc.table_name = 'users' AND tc.constraint_type = 'FOREIGN KEY' AND kcu.column_name = 'auth_user_id'")).rows[0].count, 0);
  ...
  assert.equal((await storage.findUserById(userA.id)).id, userA.id);
  assert.equal((await storage.findUserById(userA.id)).authUserId, null);
  assert.equal((await storage.findUserById(userA.id)).passwordSalt, userA.passwordSalt);
  assert.equal((await storage.findUserById(userA.id)).passwordHash, userA.passwordHash);
  assert.equal((await storage.findUserById(userB.id)).id, userB.id);
  assert.equal((await storage.findUserById(userB.id)).authUserId, null);
  assert.equal((await storage.findUserById(userB.id)).passwordSalt, userB.passwordSalt);
  assert.equal((await storage.findUserById(userB.id)).passwordHash, userB.passwordHash);
  assert.equal((await storage.findUserById(userA.id)).authUserId, null);
  const authUserIdA = crypto.randomUUID();
  const linkedA = await storage.linkUserToAuthUser(userA.id, authUserIdA);
  assert.equal(linkedA.authUserId, authUserIdA);
  assert.equal((await storage.findUserByAuthUserId(authUserIdA)).id, userA.id);
  assert.equal((await storage.linkUserToAuthUser(userA.id, authUserIdA)).id, userA.id);
  await assert.rejects(() => storage.linkUserToAuthUser(userA.id, crypto.randomUUID()), error => error.code === 'USER_AUTH_ALREADY_LINKED');
  await assert.rejects(() => storage.linkUserToAuthUser(userB.id, authUserIdA), error => error.code === 'AUTH_USER_EXISTS');
  assert.equal((await storage.findUserById(userA.id)).authUserId, authUserIdA);
  assert.equal((await storage.findUserById(userB.id)).authUserId, null);
  assert.equal(await storage.linkUserToAuthUser(crypto.randomUUID(), crypto.randomUUID()), null);
  const createdAuthUserId = crypto.randomUUID();
  const createdWithAuthInput = { id: crypto.randomUUID(), email: 'pg-auth@example.com', displayName: 'Auth', passwordSalt: 'salt', passwordHash: 'hash', authUserId: createdAuthUserId, createdAt: new Date().toISOString() };
  const createdWithAuth = await storage.createUser(createdWithAuthInput);
  assert.equal(createdWithAuth.authUserId, createdAuthUserId);
  assert.equal((await storage.findUserById(createdWithAuthInput.id)).authUserId, createdAuthUserId);
  assert.equal((await storage.findUserByEmail(createdWithAuthInput.email)).id, createdWithAuthInput.id);
  assert.equal((await storage.findUserByEmail(createdWithAuthInput.email)).authUserId, createdAuthUserId);
  assert.equal((await storage.findUserByAuthUserId(createdAuthUserId)).id, createdWithAuthInput.id);
  assert.equal((await storage.findUserByAuthUserId(createdAuthUserId)).authUserId, createdAuthUserId);
  await assert.rejects(() => storage.createUser({ id: crypto.randomUUID(), email: 'pg-auth-duplicate@example.com', displayName: 'Auth duplicate', passwordSalt: 'salt', passwordHash: 'hash', authUserId: createdWithAuth.authUserId, createdAt: new Date().toISOString() }), error => error.code === 'AUTH_USER_EXISTS');
  await assert.rejects(() => storage.createUser({ id: crypto.randomUUID(), email: 'pg-a@example.com', displayName: 'Email duplicate', passwordSalt: 'salt', passwordHash: 'hash', authUserId: crypto.randomUUID(), createdAt: new Date().toISOString() }), error => error.code === 'EMAIL_EXISTS');
  ...
  await assert.rejects(() => storage.linkUserToAuthUser(userB.id, authUserIdA), error => error.code === 'AUTH_USER_EXISTS');
  assert.equal((await storage.findCollectionById(userB.id, collectionB.id)).ownerId, userB.id);
  assert.equal((await storage.findPlaceById(userB.id, ownPlace.id)).ownerId, userB.id);
  assert.equal((await storage.findUserById(userB.id)).id, userB.id);
  assert.equal((await storage.findUserById(userB.id)).authUserId, null);
  ...
  const importUser = { id: crypto.randomUUID(), email: 'import-a@example.com', displayName: 'Import A', ...hashPassword('Current fixture password 123'), authUserId: crypto.randomUUID(), createdAt: new Date().toISOString() };
  ...
  if (importedUser.auth_user_id !== null) throw new Error('import auth mapping failed: expected NULL auth_user_id');
  if ((await importPool.query('SELECT COUNT(*)::int AS count FROM users WHERE auth_user_id IS NULL')).rows[0].count !== 2) throw new Error('import auth mapping failed: expected all users unlinked');
