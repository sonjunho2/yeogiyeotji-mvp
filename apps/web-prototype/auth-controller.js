(function () {
  'use strict';
  const issueText = {
    auth_user_not_linked: '이 이메일 인증 계정은 아직 여기였지 계정에 연결되지 않았습니다. 기존 이메일과 비밀번호로 로그인해 주세요.',
    auth_identity_conflict: '현재 비밀번호 로그인과 다른 이메일 인증 계정입니다. 이메일 인증을 종료한 뒤 다시 시도해 주세요.'
  };
  function create({ supabaseAuth, dataStore, onChange = () => {}, onAuthenticated = () => {} }) {
    ...
    const run = async (operation, fallbackMessage) => { if (state.pending) return null; setState({ pending: true, error: '', notice: '' }); try { return await operation(); } catch (error) { const code = serverCode(error); if (code === 'auth_user_not_linked' || code === 'auth_identity_conflict') setState({ mode: code === 'auth_user_not_linked' ? 'otp-unlinked' : 'otp-conflict', issue: code, error: issueText[code], pending: false }); else setState({ mode: state.mode === 'otp-verify' ? 'otp-verify' : state.mode, error: fallbackMessage, pending: false }); return null; } };
    ...
    async function cancelOtp() { if (state.pending) return; setState({ pending: true }); try { await supabaseAuth.signOut(); setState({ mode: 'login', otpEmail: '', notice: '', error: '', pending: false, issue: '' }); } catch { setState({ pending: false, error: '이메일 인증을 종료하지 못했습니다. 잠시 후 다시 시도해 주세요.' }); } }
    ...
    return { getState: () => ({ ...state }), setMode, requestOtp, verifyOtp, resendOtp, cancelOtp, handleInitialIssue };
  }
